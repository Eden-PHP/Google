...
class Base extends CoreBase
{
    ...
    public function isXml($xml)
    {
        ...
        Argument::i()->test(1, 'string', 'array', 'object', 'null');
    ...
    public function formatToXml($query)
    {
        ...
        Argument::i()->test(1, 'string', 'object');
    ...
    /**
     * Resets all the protected
     * variable that is currently
     * use.
     *
     * @return Eden\Google\Base
     */
     ...
     * @return json
     */
    protected function customPost($url, array $query = array())
    {
     ...
     * @return array
     */
    protected function delete($url, array $query = array(), $etag = false)
    {
        //if etag or developer id is needed in header
        if ($etag || !is_null($this->developerId)) {
            //set different headers for xml
            $this->headers = $this->setXmlHeaders($this->developerId, $etag);
            //add access token and version to the url
            $url = $url.'?'.self::ACCESS_TOKEN.'='.$this->token.'&v=3';
    ...
    protected function getResponse($url, array $query = array())
    {
        ...
        if (!is_null($this->developerId)) {
            $query[self::KEY] = $this->developerId;
        }

        //if needed, add api key to the query
        if (!is_null($this->apiKey)) {
            $query[self::KEY] = $this->apiKey;
        }
     ...
     * @return response
     */
    protected function patch($url, array $query = array())
    {
        //add access token to query
        $url = $url.'?'.self::ACCESS_TOKEN.'='.$this->token;
     ...
     * @return array
     */
    protected function post($url, $query, $etag = false)
    {
        ...
        if ($this->isXml($query)) {
            ...
            $url = $url.'?'.self::ACCESS_TOKEN.'='.$this->token.'&alt=json';
        }
     ...
     * @return array
     */
    protected function put($url, $query, $etag = false)
    {
        //if query is an array
        if (is_array($query)) {
            //prevent sending fields with no value
