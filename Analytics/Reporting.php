...
class Reporting extends Base
{
    ...
    public function __call($name, $args)
    {
        if (strpos($name, 'addDimension') === 0) {
            ...
            if (!isset(self::$options[$key], $args[0])) {
                return $this;
            }

            $this->dimensions[] = self::$options[$key];
            return $this;
        }
        ...
        try {
            ...
            Argument::i($e->getMessage())->trigger();
        }
    }
    ...
    public function setDimensions($dimensions)
    {
        ...
        foreach ($dimensions as $i => $dimension) {
            if (isset(self::$options[strtolower($dimension)])) {
                $dimensions[$i] = self::$options[strtolower($dimension)];
            }
        }
    ...
    public function setSort($sort)
    {
        ...
        foreach ($sort as $i => $key) {
            if (isset(self::$options[strtolower($key)])) {
                $sort[$i] = self::$options[strtolower($key)];
            }
        }
    ...
    public function setFilters($filters)
    {
        ...
        foreach ($filters as $i => $word) {
            if ($gaWord) {
                if (strpos($word, 'ga:') !== 0) {
                    $filters[$i] = 'ga:'.$word;
                    if (isset(self::$options[$word])) {
                        $filters[$i] = self::$options[$word];
                    }
                }
        ...
        $this->filters = urlencode(implode('', $filters));
    ...
    public function setSegments($segments)
    {
        ...
        foreach ($segments as $i => $segment) {
            if (isset(self::$options[strtolower($segment)])) {
                $segments[$i] = self::$options[strtolower($segments)];
            }
        }
    ...
    {
        //argument 1 must be a string
        Argument::i()->userIp(1, 'string');
        $this->userIp = $prettyPrint;
    ...
    public function getInfo($id, $startDate, $endDate, $metrics)
    {
        ...
        Argument::i()
            ...
            ->test(2, 'string', 'int')      //argument 2 must be a string or int
            ->test(3, 'string', 'int')      //argument 3 must be a string or int
            ->test(4, 'string', 'array');   //argument 4 must be a string or array
        ...
        foreach ($metrics as $i => $metric) {
            $metric = trim($metric);
            if (isset(self::$options[strtolower($metric)])) {
                $metrics[$i] = self::$options[strtolower($metric)];
            }
        }
        ...
        $query = array(
            ...
            'dimensions'    => implode(',', $this->dimensions), //optional
            'sort'          => implode(',', $this->sort),           //optional
            'filters'       => $this->filters,                      //optional
            'segment'       => implode(',', $this->segment),        //optional
            'start-index'   => $this->startIndex,       //optional
            'max-results'   => $this->maxResults,       //optional
            'fields'        => $this->fields,           //optional
            'prettyPrint'   => $this->prettyPrint,      //optional
            'userIp'        => $this->userIp,           //optional
            'quotaUser'     => $this->quotaUser);       //optional

        return $this->getResponse(self::URL_ANALYTICS_CORE_REPORTING, $query);
    }
    /* Large Data
    -------------------------------*/
